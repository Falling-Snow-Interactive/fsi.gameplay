...
namespace Fsi.Gameplay.Gameplay.ThirdPerson
{
	...
	public class ThirdPersonController : MonoBehaviour
	{
		...
		protected virtual void Awake()
		{
			...
			if (jumpAction != null) jumpAction.performed += OnJumpAction;
		}
		...
		protected virtual void UpdateMovement()
		{
			...
			if (visuals) visuals.SetMovement(velocity, false);
		}
		...
		protected virtual void ChangeMovement(Vector3 target)
		{
			...
			Vector3 testVel = flat.sqrMagnitude >= target.sqrMagnitude
		...
		protected virtual void ProvideMovementInput(Vector2 input)
		{
			...
			if (camera)
			{
				var flat = new Vector3(input.x, 0, input.y);
		...
		{
			if (velocity.sqrMagnitude > 0) ChangeRotation(velocity.normalized);
		}
